...
to spread-virus
  ask turtles with [infected? or has-shingles?]
    [ ask link-neighbors with [not infected? and not resistant? and not has-shingles? ]
...
to update-isolation-status
  ...
  [
    ...
    if isolation-progress >= isolation-duration [
      set isolating? false
...
This model is an adaptation of the virus on a network model that can be accessed from the Model Library. It demonstrates the spread of a virus through a network.  Although the model is abstract, one interpretation is that each node represents a person or possibly a household, and we are modeling the progress of a virus such as Cov-Sars-2 (Covid-19) through this network.  Each node may be in one of two states:  susceptible or infected.  In the academic literature such a model is sometimes referred to as an SI model for epidemics.

## HOW IT WORKS

Each time step (tick), each infected node (colored red) attempts to infect all of its neighbors.  Susceptible neighbors (colored green) will be infected with a probability given by the VIRUS-SPREAD-CHANCE slider. 

Infected nodes are not immediately aware that they are infected.  Only every so often (determined by the VIRUS-CHECK-FREQUENCY slider) do the nodes check whether they are infected by a virus.  This might correspond to noticing Covid symptoms or being asked to self isolate.  When the virus has been detected, all of its connections to other nodes (people) are broken. In other words, the person self-isolates or is otherwise removed and cannot infect anyone else.
...
Then press SETUP to create the network.  Press GO to run the model.  The model will stop running once the virus has completely died out or has infected everyone. If neither happens the model will run indefintitely.

The VIRUS-SPREAD-CHANCE and VIRUS-CHECK-FREQUENCY,(discussed in "How it Works" above) can be adjusted before pressing GO.

The NETWORK STATUS plot shows the number of nodes in each state (S, I, R) over time.

Instead of using the sliders, buttons are available to introduce the Rule-of-Six, wearing masks, and/or regular testing. Each of those containment strategies triggers a different kind of three-fold reduction in the capacity of the virus to spread, compared to the default parameters: The Rule-of-Six gives rise to a three-fold reduction in the average number of network connections; wearing masks leads to a three-fold reduction in the virus-spread-chance; and regular testing increases the average amount of tests three-fold. 
